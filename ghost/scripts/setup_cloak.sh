#!/bin/bash
# Cloak Installer and Configurator (generalized for nosrat)
...
CLOAK_VER=$(curl -s --max-time 15 "https://api.github.com/repos/cbeuw/Cloak/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
if [[ -z "$CLOAK_VER" ]]; then
  echo "ERROR: could not determine latest Cloak version from GitHub" >&2
  exit 1
fi

curl -fsSL -o /usr/local/bin/ck-server \
  "https://github.com/cbeuw/Cloak/releases/download/$CLOAK_VER/ck-server-linux-amd64"
