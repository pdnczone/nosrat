#!/bin/bash
# Resource Optimization and Monitoring Setup (generalized for nosrat)
...
# Enable zRAM (Ubuntu-only, gracefully skip on other distros)
...
# Create swap file (only if no swap exists or none active)
...
# Cron jobs (resource monitor + path rotation)
INTERVAL="${PATH_ROTATION_INTERVAL:-6}"
(crontab -l 2>/dev/null | grep -v "resource-monitor.sh" | grep -v "rotate-path.sh"; \
 echo "*/5 * * * * /usr/local/bin/resource-monitor.sh"; \
 echo "0 */$INTERVAL * * * /usr/local/bin/rotate-path.sh") | crontab -
